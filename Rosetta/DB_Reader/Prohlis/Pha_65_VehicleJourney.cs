...
{
    internal class Pha_65_VehicleJourney : Pha_GA_GenericAutomated
    {
        public Pha_65_VehicleJourney() : base(65)
        {
        }

        protected override string _HEader_ConstString =>
            "VEHJOURNEY:No;Dep;LineName;LineRouteName;DirectionCode;TimeProfileName;FromTProfItemIndex;ToTProfItemIndex;Name";
        ...
        protected override IEnumerable<Tuple<int, int?>> _OwnObj_IdentsOnly()
        {
            return _phs.PuT.Lines.SelectMany(l => l.Strecken).SelectMany(lr => lr.Profiles)
                .SelectMany(lp => lp.Kurse.Select(k => new Tuple<int, int?>(k.ObjectId, null)));
        }

        protected override IEnumerable<BlaBinResultRow> _OwnObj_AsString()
        {
            return _phs.PuT.Lines.SelectMany(l => l.Strecken).SelectMany(lr => lr.Profiles).SelectMany(lp =>
                lp.Kurse.Select(k =>
                    new BlaBinResultRow
                    {
                        Content = k.Core(),
