...
{
    internal class Pha_42_Stops : Pha_GA_GenericAutomated
    {
        public Pha_42_Stops() : base(42)
        {
        }

        protected override string _HEader_ConstString => "STOP:NO;CODE;NAME;TYPENO;XCOORD;YCOORD";
        ...
        protected override IEnumerable<BlaBinResultRow> _OwnObj_AsString()
        {
            ...
            {
                EntityID = 42, OrdNum = psi.Id, RelObjID = null,
                Content =
                    $"{psi.Id};{psi.Name};{psi.Name};{psi.TypeNo};{psi.X.ToString("F2", CultureInfo.InvariantCulture)};{psi.Y.ToString("F2", CultureInfo.InvariantCulture)}"
            });
        }
    }
}