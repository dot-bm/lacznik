...
{
    internal class Pha_66_VehJourneySection : Pha_GA_GenericAutomated
    {
        public Pha_66_VehJourneySection() : base(66)
        {
        }

        //VehJourneyNo;No;FromTProfItemIndex;ToTProfItemIndex
        ...
        protected override IEnumerable<Tuple<int, int?>> _OwnObj_IdentsOnly()
        {
            return _phs.PuT.Lines.SelectMany(l => l.Strecken).SelectMany(lr => lr.Profiles)
                .SelectMany(lp => lp.Kurse.Select(k => new Tuple<int, int?>(k.ObjectId, 0)));
        }
        ...
        protected override IEnumerable<BlaBinResultRow> _OwnObj_AsString()
        {
            return _phs.PuT.Lines.SelectMany(l => l.Strecken).SelectMany(lr => lr.Profiles).SelectMany(lp =>
                lp.Kurse.Select(k =>
                    new BlaBinResultRow
                    {
                        Content = $"{k.ObjectId};1;1;{lp.HalteStelleAnzahl}",
                        EntityID = EntityID,
                        OrdNum = lp.ObjectId,
                        RelObjID = 0
                    }));
