...
{
    public abstract class Rck_LinkCollcetor_OfT<T> : Rck_Generic where T : KdzInfraLink
    {
        protected List<T> _ilist = new List<T>();
        protected int[] _johEntries;

        public Rck_LinkCollcetor_OfT(int johEntityId, int johPrT, int johPuT)
        {
            _johEntries = new[] {johEntityId, johPrT, johPuT};
        }
        ...
        public override bool Prepare()
        {
            ...
            try
            {
                ...
                _platform.Reporter.AddMessage(new RoReMessage
                    {Message = "Wczytano definicje krawędzi", EvType = RoReEventType.Info});
                ...
                b = _ilist.GroupJoin(stage2, l => l.Id, sy => sy.obj,
                    (l, sy) => { return l.AddTSyses(sy.Select(v => v.tsys)); }).All(u => u);
            }
            ...
            {
                _platform.Reporter.AddException(e, GetType().Name);
                b = false;
            }
        ...
        protected abstract T _MakeLink(SqlDataReader rea);
